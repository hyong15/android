...
public abstract class PlayActivity extends BaseActivity implements
    ...
    protected TextView pressToTalk;// 按住说话
    ...
    protected TextView talkMode;// 对讲模式提示
    ...
    protected TextView horfuncTalkMode;// 对讲模式提示
    ...
    protected int channelOfChannel;
    protected ArrayList<Device> deviceList;
    // 设备分组
    protected String deviceGroup;
    protected String functions[];// 功能数组
    protected boolean isVideoConnected;// 判断视频是否连接成功
    ...
    protected void initUi() {
        ...
        pressToTalk = (TextView) findViewById(R.id.textone);
        ...
        talkMode = (TextView) findViewById(R.id.talkmode);
        ...
        horfuncTalkMode = (TextView) findViewById(R.id.talkmodeh);
        ...
        if ("C".equals(deviceGroup)) {
            functions = getResources().getStringArray(
                    R.array.array_play_functions_cat);
        } else {
            if (Consts.PLAY_AP == playFlag) {
                functions = getResources().getStringArray(
                        R.array.array_play_functions_ap);
            } else {
                functions = getResources().getStringArray(
                        R.array.array_play_functions_common);
            }
        }
        ...
        // 云台速度调整按钮
        ytSeekLayout = (RelativeLayout) ytLayout
                .findViewById(R.id.ytspeedseekbarlayout);// 云台速度调整布局
    ...
    protected void voiceCallSelected(Boolean selected) {
        if (selected) {// 选中
            ...
            function.setVisibility(View.VISIBLE); // 显示远程回放等功能
     ...
     * @param channel 通道
     */
    protected void deployShareLink(boolean status, Channel channel) {
        // 如果功能列表中没有分享功能,不进行后面的处理
        if (!contains(functions, "e")) {
            return;
        }
        ...
        if (status) {
            ...
            if (!"C".equals(deviceGroup)) {
                // 修改分享链接的文字 -> 分享/管理视频
                resId = R.string.str_share_link_demo;
                // 本地模式(登录/访客判断)
                if (Boolean.valueOf(statusHashMap.get(Consts.LOCAL_LOGIN))) {
                    MyLog.v(TAG, "--local mode--");
                    status = false;
                } else {
                    // 设备判断(判断是否是IPC设备)
                    if (channel.getParent().getType() != Consts.DEVICE_TYPE_IPC) {
                        MyLog.v(TAG, "--not ipc device--");
                        status = false;
                    }

                    // 语言判断(区分国外使用中文的情况)
                    if (ConfigUtil.getServerLanguage() != 1) {
                        MyLog.v(TAG, "--language error--");
                        status = false;
                    }
                }
            }

            functionListAdapter.displayShareLinkSwitch(false, resId);
        } else {
            // 隐藏分享链接
            functionListAdapter.displayShareLinkSwitch(false, resId);
        }
