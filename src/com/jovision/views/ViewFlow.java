...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    public int activityTag = 0;// 0：mainActivity（可滚动） 1：guidActivity 2：新品详细信息
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    if (0 == activityTag) {// mainActivity的单击事件
                        // if (null != BaseApp.mainHandler) {
                        // if (JVMainActivity.openNewPro) {// 已经打开什么也不做
                        //
                        // } else {
                        // JVMainActivity.openNewPro = true;
                        // Message msg = BaseApp.mainHandler.obtainMessage();
                        // Bundle data = new Bundle();
                        // // Log.e("CurrentAdapterIndex",
                        // // mCurrentAdapterIndex+"");
                        // data.putInt("CurrentAdapterIndex",
                        // mCurrentAdapterIndex);
                        // msg.what = JVConst.NEW_PRO_DETAIL;
                        // msg.setData(data);
    ...
    private void setVisibleView(int indexInBuffer, boolean uiThread) {
        ...
        if (1 == activityTag) {// 帮助界面不自动滚动，guideActivity
            ...
            // Message msg = BaseApp.mainHandler.obtainMessage();
            // Bundle data = new Bundle();
            // // Log.e("CurrentAdapterIndex", mCurrentAdapterIndex+"");
            // data.putInt("CurrentAdapterIndex", mCurrentAdapterIndex);
            // msg.what = JVConst.NEW_PRO_CHANGE_POSITION;
            // msg.setData(data);
            // BaseApp.mainHandler.sendMessage(msg);
            //
            // }
    ...
    private void postViewSwitched(int direction) {
        ...
        } else if (direction == -5) {// 到最后一个自动移动到第一个
    ...
    class AdapterDataSetObserver extends DataSetObserver {
        ...
        public void onChanged() {
            ...
            if (v != null) {
                // Log.v("mAdapter.getCount()", mAdapter.getCount()+"");
                // Log.v("mSideBuffer", mSideBuffer+"");
                try {
