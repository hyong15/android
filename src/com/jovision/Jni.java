...
public class Jni {
    ...
    /**
     * 搜索局域网服务端，参考 {@link JVSUDT#JVC_StartLANSerchServer(int, int)} 搜索全部
     * 
    ...
    /**
     * [METHOD] 2015-06-03 删除设备已经设置的小助手 HelperRemove [IN] pGroup
     * 编组号，编组号+nYSTNO可确定唯一设备 [IN] NYST 搜索具有某云视通号码的设备，>0有效 [RETURN] no
     */

    public static native boolean HelperRemove(String gid, int no);
    ...
    /**
     * 发送整数数据，参考 {@link JVSUDT#JVC_SendPlaybackData(int, byte, int, int)} 实际调用
    ...
    /**
     * 翻转视频，参考 {@link JVSUDT#JVC_TurnVideo(int, byte, byte[])} // [Neo] TODO 未验证
     * 
     * @param window 窗口索引
     * @param uchType
     * @param cmd
    ...
    /**
     * 设备升级，参考 {@link JVSUDT#JVC_DeviceUpdate(int, byte)} // [Neo] TODO 未验证
     * 
     * @param window 窗口索引
     * @param uchType
     */
    ...
    /**
     * 启用底层日志打印，参考 {@link JVSUDT#JVC_EnableLog(boolean)} // [Neo] TODO 未验证
     * 
     * @param enable
     */
    ...
    /**
     * 设置日志和提示信息区域语言，参考 {@link JVSUDT#JVC_SetLanguage(int)} // [Neo] TODO 未验证
     * 
     * @param typeId 参考 {@link JVNetConst#JVN_LANGUAGE_ENGLISH}，
     ...
     * 设置缓冲帧个数
     * 
     * @param min 最小缓冲帧个数：0~25 5
     * @param buffer 目标缓冲数量：25~1000 50
     * @return
     */
    ...
    // 超时时间nTimeOutS 单位秒 3s
    public static native String getBatchChannelCount(int nTimeOutS,
            String resJson);

    /**
     * 2015-5-29 nMtu可以设置的值只有700,1400（七百，1千4百）
     * 
     * @param nMtu
     * @return 1:成功 0：失败
     */
    ...
    /**
     * 2015-6-11 STOP lansearch return：1:success 0:failed
     */
    public static native int StopMobLansearch();

    // JNIEXPORT jint JNICALL Java_com_jovision_Jni_StopMobLansearch(JNIEnv
    // *env,
    // jclass clazz);

    /**
     * 2015-6-16 停止小助手 停止小助手功能 JNIEXPORT jint JNICALL
     * Java_com_jovision_Jni_StopHelp(JNIEnv *env, jclass clazz); 设置mtu的调用步骤：
     * 1、调用停止小助手函数 StopHelp(); 2、设置mtu SetMtu(int mtu); 3、打开小助手函数
     * enableLinkHelper 4、把所有设备设置到小助手函数中。setLinkHelper
