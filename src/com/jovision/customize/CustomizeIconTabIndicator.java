...
public class CustomizeIconTabIndicator extends LinearLayout {
    ...
    private void animateToTab(final int position) {
        final View tabView = this.getChildAt(position);
    ...
    private void addTab(int index, char tag, CharSequence text, int iconResId,
        ...
        this.addView(tabView, new LinearLayout.LayoutParams(0, MATCH_PARENT, 1));
    }
    ...
    private void setCurrentItem(int item) {
        ...
        final int tabCount = this.getChildCount();
    ...
    private void clickEvents(TabView tabView) {
        ...
        if (oldSelected == newSelected && mTabSelectedListener != null) {
            mTabSelectedListener.onTabReselected(newSelected, tag, isCustomize);
        } else {
            // 点击定制Tab后不选中
            if (!isCustomize) {
                setCurrentItem(newSelected);
            }
            mTabSelectedListener.onTabSelected(newSelected, oldSelected, tag,
                    isCustomize);
        }
    }
    ...
    // ---------------------------------------------------
    public void updateIndicator(int which, int msgCount, boolean show, int count) {
        TabView tabView = (TabView) mIndicators[which];
        RelativeLayout rlty = (RelativeLayout) tabView
                .findViewById(R.id.tab_info);

        // 最后一个Tab的位置
        int lastPosition = mIndicators.length -mIndicators.length+ 1;
        switch (which) {
