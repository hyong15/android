...
public class AccountUtil {
     ...
     * @return
     */
    // public static int isUserExsit1(String userName) {
    // int res = -1;
    ...
    public static int isUserExsitV2(String userName) {
        int res = -1;
        res = JVACCOUNT.IsUserExistsV2(userName);
        MyLog.v("isUserExsitV2--", userName + "-----|||||" + res + "");
        return res;
    }
    ...
    public static int userRegisterV2(User user) {
        ...
        try {
            reqObj.put("account_type", user.getUserType());
            reqObj.put("passwd", user.getUserPwd());
            reqObj.put("mail", user.getUserEmail());
            reqObj.put("phone", user.getUserPhone());
            reqObj.put("location", ConfigUtil.getCountry());
            reqObj.put("custom_type", Consts.APP_NAME);
        } catch (JSONException e) {
            // TODO 自动生成的 catch 块
            e.printStackTrace();
        }
        res = JVACCOUNT.UserRegisterV2(reqObj.toString());
        MyLog.v("userRegisterV2--", "-----|||||" + res + "");
        return res;
    }

    /**
     * 集群服务器获得此用户需要上线的账号服务云
     * 
     * @return
     */
    public static int ServerAddrFromCluster(String name) {
        int ret;
        if (name == null) {
            return -9002;

        }
        ret = JVACCOUNT.SetServerAddrFromCluster(name);
        if (ret != 0) {
            Log.e("onLoginProcessV2", "SetServerAddrFromCluster failed:" + ret);
            return ret;
        }
        return ret;
    }
     ...
     * @return
     */
    public static int UserLoginV2(String name, String pass, Context con,
            int startHeartBeat) {
        int res = -1;
        ClientBean cb = new ClientBean();
        cb.setPlatformType(1);
        cb.setProductType(Consts.PRODUCT_TYPE);
        String language = con.getResources().getConfiguration().locale
                .getCountry();
        ...
        res = JVACCOUNT.UserLoginV2(name, pass, cb, startHeartBeat);
        Log.e("userLoginV2--", "-----|||||" + res + "");
        return res;
    }

    public static int AddrFromClusterAndLogin(String name, String pass,
            Context con, int startHeartBeat) {
        int ret;
        ret = ServerAddrFromCluster(name);
        if (ret != 0) {
            Log.e("onLoginProcessV2", "SetServerAddrFromCluster failed:" + ret);
            return ret;
        } else {
            ret = UserLoginV2(name, pass, con, startHeartBeat);
        }
    ...
    // String pwd, String urlLgServ, String urlStServ, int startHeartBeat) {//
    // 1:起心跳
    // // 0：不需要起心跳
    //
    ...
    // }
    //
    // JSONObject initRetObj = new JSONObject();
    //
    // // int ret =
    // //
    // JVACCOUNT.ConfigClusterServer("http://60.216.75.26:9999/jenkins/job/IPC/ws/cluster_test.txt");
    // // if(ret != 0){
    // // Log.e("onLoginProcessV2", "ConfigClusterServer failed:"+ret);
    // // try {
    // // initRetObj.put("result", ret);
    // // initRetObj.put("arg1", ret);
    // // initRetObj.put("arg2", ret);
    // // initRetObj.put("data", "");
    // // } catch (JSONException e) {
    // // // TODO Auto-generated catch block
    // // e.printStackTrace();
    // // }
    // // return initRetObj.toString();
    // // }
    // int ret;
    // ret = JVACCOUNT.SetServerAddrFromCluster(userName);
    // if (ret != 0) {
    // Log.e("onLoginProcessV2", "SetServerAddrFromCluster failed:" + ret);
    // try {
    // initRetObj.put("result", ret);
