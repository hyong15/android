...
public class PlayUtil {
    ...
    public static Bitmap[] h264Bitmap = {
    ...
    public static boolean broadCast(Context con) {
        ...
        if (!ConfigUtil.is3G(con, false)) {// 非3G加广播设备
            canBroad = true;
            int res = Jni.searchLanDevice("", 0, 0, 0, "", 2 * 1000, 2);
        }
        return canBroad;
    }
     ...
     * @return
     */
    public static void refreshAllDevOnlineState(ArrayList<Device> devList,
            HashMap<String, Integer> onLineMap) {
        for (Device dev : devList) {
            dev.setOnlineStateNet(onLineMap.get(dev.getFullNo()));
            MyLog.v(TAG,
                    "refreshAllDevOnlineState-devFullNo=" + dev.getFullNo()
                            + ";online=" + dev.getFullNo());
        }
    }
     ...
     * @param cmd
     */
    public static void sendCtrlCMD(int index, int cmd, int speed) {
    ...
    public static void sendCtrlCMDLongPush(final int index, final int cmd,
            final boolean start, final int speed) {
        new Thread() {
            @Override
            public void run() {
                if (start) {
                    ...
                    data[3] = (byte) speed;
                    // 云台命令
                    Jni.sendBytes(index, (byte) JVNetConst.JVN_CMD_YTCTRL,
                            data, 4);
                } else {
                    // 如果不是自动命令 发完云台命令接着发一条停止
                    byte[] data1 = new byte[4];
     ...
     * @param stop
     */
    public static void setYTSpeed(final int index, final int speed) {
        new Thread() {
            @Override
            public void run() {
                byte[] data = new byte[4];
    ...
    /**
     * 2015-06-03 删除所有设备的小助手
     * 
     * @param deviceList
     */
    public static void deleteHelperAll(ArrayList<Device> deviceList) {

        if (MySharedPreference.getBoolean(Consts.MORE_LITTLEHELP, true)) {
            if (null != deviceList && 0 != deviceList.size()) {
                for (Device device : deviceList) {
                    try {
                        String devNum = device.getFullNo();
                        String gid = ConfigUtil.getGroup(devNum);
                        int no = ConfigUtil.getYST(devNum);
                        // 移除小助手
                        Jni.HelperRemove(gid, no);
                        MyLog.v(TAG, "deleteHelper-" + devNum);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    public static void setHelperToList(ArrayList<Device> deviceList) {
        ...
        if (MySharedPreference.getBoolean(Consts.MORE_LITTLEHELP, true)) {
            ...
            if (null == helperMap || 0 == helperMap.size()) {
                ...
                for (Device device : deviceList) {
                    if (helperMap.containsKey(device.getFullNo())
                        ...
                        // 已设置小助手
                        MyLog.v(TAG,
                                "setHelperToList-已设置小助手:" + device.getFullNo());
                    } else {
                        MyLog.v(TAG,
                                "setHelperToList-还没设置小助手:" + device.getFullNo());
                        try {
                            object = new JSONObject();
                            object.put("gid", device.getGid());
                            object.put("no", device.getNo());
            ...
            if (!"".equalsIgnoreCase(array.toString())) {
                MyLog.v(TAG, "setHelperToList-需要设置小助手:" + array.toString());
     ...
     * @param channleList
     */
    public static void disConnectAll(ArrayList<Channel> channelList) {
        if (null != channelList && 0 != channelList.size()) {
            try {
                int size = channelList.size();
                for (int i = 0; i < size; i++) {
                    if (channelList.get(i).isConnected()
                            || channelList.get(i).isConnecting()) {

                        // ((View) mLastPlayView.getParent())
                        // .setBackgroundColor(Color.BLACK);
                        MyLog.e("disconnect", channelList.get(i).toString()
                                + "");
                        if (1 == channelList.get(i).getVipLevel()) {
                            Jni.shutdownRTMP(channelList.get(i).getIndex());
                        } else {
