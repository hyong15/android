...
public class DeviceUtil {
                            ...
                            if (null != dlist && 0 != dlist.length()) {
                                for (int i = 0; i < dlist.length(); i++) {
                                    ...
                                    if (null != obj) {
                                        ...
                                        // 获取通道数量是否正确
                                        dev.setChannelBindFlag(obj
                                                .optInt(JVDeviceConst.JK_DEVICE_BIND_TYPE));
                                        // 云存储开关
                                        dev.setCloudEnabled(obj
                                                .optInt(JVDeviceConst.JK_CLOUD_STORAGE_FLAG,
                                                        -1));
                                        deviceList.add(dev);
     ...
     * @param deviceList
     */
    public static int refreshOnlineState(ArrayList<Device> deviceList) {
        int getRes = -1;
                                ...
                                for (int i = 0; i < resArray.length(); i++) {
                                    ...
                                    if (null != devObj) {
                                        ...
                                        int onlineState = devObj
                                                .optInt("dstat");
                                        onLineMap.put(dGuid, onlineState);
        ...
        if (null != deviceList && 0 != deviceList.size() && null != onLineMap
                && 0 != onLineMap.size()) {

            PlayUtil.refreshAllDevOnlineState(deviceList, onLineMap);
        }
        return getRes;
    }
    ...
    public static Device addDevice2(Device device, String userName,
        ...
        // 请求参数示例
        // 2015-04-16 增加字段"dbt" 0.获取通道数正确，1.获取通道数错误
        /**
         * {"dvpassword":"","dvusername":"admin","lpt":1,"pv":"2.0",
         * "mt":2015,"dguid"
         * :"S230829711","mid":12,"sid":"sidtest","dcs":2，,"dbt":0}
         */
        ...
        // JK_DEVICE_BIND_TYPE : <int> (设备绑定状态 0正常 1通道数不对)
        ...
        try {
            ...
            jObj.put(JVDeviceConst.JK_DEVICE_CHANNEL_SUM, device
                    .getChannelList().size());
            jObj.put(JVDeviceConst.JK_DEVICE_BIND_TYPE,
                    device.getChannelBindFlag());
        } catch (Exception e1) {
            e1.printStackTrace();
        }
                ...
                try {
                    ...
                    if (null != temObj) {
                        ...
                        if (0 == rt) {
                            ...
                            // 云存储开关
                            device.setCloudEnabled(devObj.optInt(
     ...
     * @return
     */
    public static int modifyChannalNum(String devYstNum, int devChaNum) {
        int res = -1;
        // 请求参数示例
        /**
         * request: { JK_MESSAGE_ID : <int> , JK_MESSAGE_TYPE : <int> ,
         * (FIX_DEVICE_CHANNEL 2061) JK_PROTO_VERSION : <string> , (1.0)
         * JK_LOGIC_PROCESS_TYPE : <int> , (DEV_INFO_PRO 1) JK_SESSION_ID :
         * <string> , JK_DEVICE_GUID : <string> , JK_DEVICE_CHANNEL_SUM : <int>
         * (通道数) }
         */
        JSONObject jObj = new JSONObject();
        try {
        ...
        MyLog.v("modifyChannalNum---request", jObj.toString());

        /**
         * 请求实例： {"mid":13,"mt":2061,"pv":"1.0", "lpt":1,"sid":"sidtest",
         * "dguid":"A362","dcs":4}
         */
        /**
         * response: { JK_MESSAGE_ID : <int> , JK_MESSAGE_TYPE : <int> ,
         * (FIX_DEVICE_CHANNEL_RESPONSE 2062) JK_RESULT : <int> }
         */
        String requesRes = JVACCOUNT
                .GetResponseByRequestDeviceShortConnectionServerV2(jObj
                        .toString());
        ...
        if (ret == 0) {
            String result = respObject.optString("resp", "");
            MyLog.v("modifyChannalNum---result", result);
            if (null != result && !"".equalsIgnoreCase(result)) {
                try {
                    JSONObject temObj = new JSONObject(result);
                    if (null != temObj) {
        ...
        try {
            switch (settingTag) {
                ...
                case JVDeviceConst.DEVICE_TF_SWITCH:// TF卡存储
                    ...
                    // TF卡开关
                    paramObj.put(JVDeviceConst.JK_TF_STORAGE_SWITCH, state);// 0关
                    // 1开
                    break;
                ...
                case JVDeviceConst.DEVICE_ALARTM_BABY_SWITCH:// baby模式
                    ...
                    // TF卡开关
                    paramObj.put(JVDeviceConst.JK_DEVICE_BABY_MODE, state);// 0关
                    // 1开
                    break;
            }
        } catch (Exception e) {
    ...
    /**
     * 2015-1-4 获取演示点网页地址 (2015-01-30 暂停使用此接口)
     * 
     * @param
     * @return ArrayList<Device> 设备列表
     */
    ...
    /**
     * 2014-11-20 获取广告信息 2015-05-29 增加各种版本号
     * 
     * @param
     * @return ArrayList<Device> 设备列表
     */
    public static ArrayList<AD> getADList(int adVersion) {
                            ...
                            if (null != dlist && 0 != dlist.length()) {
                                for (int i = 0; i < dlist.length(); i++) {
                                    ...
                                    if (null != obj) {
                                        ...
                                        ad.setAdImgVersion(obj
                                                .getInt(JVDeviceConst.JK_AD_PIC_VER));
                                        ad.setAdLinkVersion(obj
                                                .getInt(JVDeviceConst.JK_AD_LINK_VER));
                                        ad.setVersion(adver);
                                        adList.add(ad);
                                    }
                                }
        ...
        if (null != adList && 0 != adList.size()) {
            Collections.sort(adList);// 2015-06-04 广告位按照index正序排序
            // http://www.cnblogs.com/wentiertong/archive/2011/03/07/1973698.html
        }
        return adList;
    }
                ...
                try {
                    ...
                    if (null != temObj) {
                        ...
                        if (0 == rt) {
                            ...
                            webUrl.setShareUrl(temObj
                                    .optString(JVDeviceConst.JK_WEB_SHARE_URL));// 视频分享到广场
                            webUrl.setNoteListUrl(temObj
                                    .optString(JVDeviceConst.JK_WEB_TOPIC_LIST));// 个人中心帖子列表地址
                            webUrl.setNoteDetailUrl(temObj
                                    .optString(JVDeviceConst.JK_WEB_TOPIC_DETAIL));// 个人中心帖子详情地址
                            ...
                            webUrl.setShopSwitch(temObj
                                    .optInt(JVDeviceConst.JV_WEB_SHOP_FLAG));// 商城开关
                            webUrl.setShareSwitch(temObj
                                    .optInt(JVDeviceConst.JK_WEB_SHARE_FLAG));// 视频分享到广场开关
                            webUrl.setNoteSwitch(temObj
                                    .optInt(JVDeviceConst.JK_WEB_TOPIC_FLAG));// 个人中心帖子开关
                        } else {
                            webUrl = null;
                        }
                    }
