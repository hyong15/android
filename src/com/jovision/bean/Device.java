...
public class Device {
    ...
    private boolean homeIPCMergeCode = false;// 2015.4.17是否融合后的家用 true ：融合 false
                                             // ：非融合

    private boolean homeIPCFlag = false;// 2015.4.17是否家用ipc true ：家用 false ：非家用

    private int motionSensitivity = 0;// 2015.5.4 移动侦测灵敏度

    private boolean hasSdCard = false;// 2015.5.4 设备是否有sd卡 true:有sd卡 false：无sd卡

    private int nTotalSize = 0;// 2015.5.4 sd卡的容量大小 单位M

    private int nUsedSize = 0;// 2015.5.4 sd卡已用空间

    private int nStatus = 0;// 2015.5.4 存储状态

    private boolean newH411 = false;// 2015.5.4 true：增加回声抑制的新h411
                                    // false：未增加回声抑制的老h411
    ...
    public static Device fromJson(String string) {
        ...
        try {
            ...
            dev.setCloudEnabled(ConfigUtil.optInt(object, "cloudEnabled", -1));// 云存储
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void setnTotalSize(int nTotalSize) {
    ...
    public void setnUsedSize(int nUsedSize) {
    ...
    public void setNewH411(boolean newH411) {
        this.newH411 = newH411;
    }
