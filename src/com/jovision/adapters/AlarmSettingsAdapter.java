...
public class AlarmSettingsAdapter extends BaseAdapter {
    ...
    public AlarmSettingsAdapter(Context context,
            List<AlarmSettingsItemBean> listData) {
        ...
        try {
            if (dataObj.getIsTag()) {
                ...
                if (!enabled) {
                    ...
                    tv_title.setTextColor(_context.getResources().getColor(
                            R.color.more_fragment_color7));
                    tv_tips.setTextColor(_context.getResources().getColor(
                            R.color.more_fragment_color7));
                    int func_switch = dataObj.getSwitch();
                    if (0 == func_switch) {
                        // 关闭
                        img_func_sw
                                .setBackgroundResource(R.drawable.morefragment_normal_disabled_con);
                    } else if (1 == func_switch) {
                        // 打开
                        img_func_sw
                                .setBackgroundResource(R.drawable.morefragment_selected_disabled_icon);
                    } else {
