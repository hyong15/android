...
public class JVACCOUNT {
     ...
     * @return
     */
    public static native int IsUserExistV2(String userName);
     ...
     * 2-用户注册V2 2015-04-27
     * 
     * @param u注册账号类型 (ACCOUNT_MAIL = 1邮箱 ，ACCOUNT_PHONE = 2手机)
     *            用户密码（由上层直接传入加密的密码） 邮箱
     * @param [in] 手机号
     * @param [in] 地区字符串 （例如：中国 美国 英国 通过新浪接口获得）
     * @param [in] 客户端的定制类型（可选），定制版本使用
     * @return
     */
    public static native int UserRegisterV2(int account_type, String password,
            String mail, String phone, String location, String customtype);
    ...
    public static native int UserLoginV2(String userName, String pwd,
            Object clientInfo, int heartbeat);
     ...
     * 用户切换地区
     */
    public static native int DataMigration(String userName, String location);

    /**
     * 设备接口短连接V2，调用.该接口在V3.3.1以上版本账号库内部已全面替换
     * 
     * @param request
     * @param response
     * @return
    ...
    /**
     * 设备接口长连接V2，调用.该接口在V3.3.1以上版本账号库内部已全面替换
     * 
     * @param request
     * @param response
     * @return
    ...
    // 账号库新架构接口
    public static native int ConfigClusterServer(String clientIp);

    public static native int SetServerAddrFromCluster(String account);

    public static native int UserRegisterV2(String reqJson);

    public static native int IsUserExistsV2(String accountName);

    public static native int ReportUserFlow(String account, String guid,
            int channel, int flow_type, int flow_subtype, int flow);
}
